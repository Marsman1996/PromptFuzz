...
STALIB_NAME=libming.a
DYNLIB_NAME=libming.so
DIR=$(pwd)
set -Eeo pipefail
set -x
...
function download() {
    ...
    cd $SRC
    cp -r /prompt_fuzz/data/libming/libming ./
...
function build_lib() {
    ...
    ./autogen.sh
    cd $WORK/build
    $SRC/libming/configure --prefix=$WORK --enable-static --enable-shared --disable-freetype
    make && make install
...
function copy_include() {
    ...
    cp $WORK/include/* ${LIB_BUILD}/include/
}
