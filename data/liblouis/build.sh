...
function download() {
    apt-get update &&
        # apt-get -y upgrade &&
        apt-get -y install autoconf libtool libyaml-dev

    cd $SRC
    cp -r /prompt_fuzz/data/liblouis/liblouis ./
...
function build_lib() {
    ...
    ./autogen.sh
    cd $WORK/build
    $SRC/liblouis/configure --prefix=$WORK --enable-static --enable-shared
...
function copy_include() {
    ...
    cp $WORK/include/liblouis/* ${LIB_BUILD}/include/
}
